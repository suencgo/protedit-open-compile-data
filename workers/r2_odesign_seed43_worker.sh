#!/usr/bin/env bash
# r2 COPY of $PE/scripts/run_odesign_pilot_worker.sh (round-2 generation,
# seed 43, output must go to runs/<m>/odesign/r2/shard-XXXX). Original untouched.
set -Eeuo pipefail
...
case "/$output_dir/" in
  */r1/*) echo "FATAL: r2 worker refuses r1 output path: $output_dir" >&2; exit 65;;
esac
# --------------------------------------------------------------------------
...
"$env_dir/bin/python" scripts/inference.py \
  ...
  exp.exp_name=odesign_llm_r2_seed43_20260922 \
  'exp.seeds=[43]' \
